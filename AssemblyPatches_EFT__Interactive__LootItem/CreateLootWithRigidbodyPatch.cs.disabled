...
namespace SPTLootFetching.AssemblyPatches_EFT__Interactive__LootItem {
    public class CreateLootWithRigidbodyPatch : ModulePatch {
        protected override MethodBase GetTargetMethod () {
            return typeof(EFT.Interactive.LootItem)
                .GetMethod(nameof(EFT.Interactive.LootItem.CreateLootWithRigidbody), BindingFlags.Public | BindingFlags.Static)
                .MakeGenericMethod(typeof(EFT.Interactive.LootItem));
        }

        [PatchPostfix]
        public static void Postfix (ref EFT.Interactive.LootItem __result) {
            _ = __result.gameObject.GetOrAddComponent<LootNameLabelComponent>();
        }
    }
}
